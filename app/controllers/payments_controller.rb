class PaymentsController < ApplicationController
  ...
  def create
    ...
    if params["payment"].present? && params["payment"]["cash"].present? && params["payment"]["cash"] == "true"
      ...
      @kit.save
      @payment = Payment.new(approved: true, status: "approved", cash: true, dni: cash_params["dni"], date_approved: Time.now.to_s, operation_type: "cash", order: @order, payment_method_id: "cash", payment_type_id: "cash", status_detail: "approved", total: @kit.price * @order.amount * (100 - @store.discount) / 100)
      @payment.save
      authorize @payment
